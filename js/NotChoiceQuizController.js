quizControllers.controller('NotChoiceQuizCtrl', ['$scope', '$window', '$routeParams', '$location', 'Facebook', '$route', 'quizManager', function ($scope, $window, $routeParams, $location, Facebook, $route, quizManager) {
    quizManager.loginCallbacks = {
        success: function (user, network) {
            var ticks = quizManager.currentTicks();
            console.log("currentTicks:" + ticks);

            var user_query = KiiQuery.queryWithClause(KiiClause.and(KiiClause.notEquals("kind", "normal"), KiiClause.lessThan("due", ticks)));
